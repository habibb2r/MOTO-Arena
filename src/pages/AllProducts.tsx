...
const AllProducts = () => {
  ...
  return (
    <div className="px-5 pb-5">
      ...
      />
      {/* seacrh and filter  */}
      <div className="flex  rounded-box justify-between shadow-md items-center my-5 px-4 ">
        <div className="px-5 md:w-[400px] w-[250px] flex justify-center items-center py-5">
          <Search
        ...
        <div className=" md:w-1/2 w-1/3  flex justify-end">
          <button onClick={() => setOpenFilter(!openFilter)}>
            {" "}
            <img className="h-[50px]" src={filterLogo} alt="" />
          </button>
        </div>
      </div>
      {/* show card */}
      <div className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2  gap-5">
        {!isLoading || !filterLoading ? (
          getAllProducts?.data?.map((product: TProduct, idx: any) => (
            <ProductCard
              key={idx}
              name={product?.name}
